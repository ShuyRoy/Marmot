package me.jinheng.cityullm.ui.slideshow
...
import me.jinheng.cityullm.databinding.FragmentSlideshowBinding
import me.jinheng.cityullm.models.ModelInfo
import me.jinheng.cityullm.models.ModelOperation
import me.jinheng.cityullm.ui.gallery.bytesToGigabytes
...
//            }
            ...
            dynamicTextView.setOnLongClickListener {
                ...
                popupMenu.menuInflater.inflate(R.menu.menu_options2, popupMenu.menu)
                popupMenu.setOnMenuItemClickListener { menuItem ->
                    when (menuItem.itemId) {
                        R.id.action_download -> {
                            val modelName = (dynamicTextView.text as String).substringBefore("(")
                            // 处理删除操作
                            Toast.makeText(context, "Download " + modelName, Toast.LENGTH_SHORT).show()
//                            ModelOperation.downloadModel(modelName)
                            ModelOperation.downloadModelAsync(modelName, dynamicTextView)
                            true
                        }
                        R.id.pause_download -> {
                            ModelOperation.pauseDownload()
                            true
                        }
                        R.id.cancel_download -> {
                            ModelOperation.cancelDownload()
                            true
                        }
                        R.id.resume_download -> {
                            ModelOperation.resumeDownload()
                            true
                        }
                        else -> false
                    }
